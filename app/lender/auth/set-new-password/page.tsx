...
import { userAuth } from "@/lib/api/userAuth"; // <-- Use your lender auth API
...
export default function LenderSetNewPasswordPage() {
  ...
  useEffect(() => {
    if (!userId) {
      ...
      setTimeout(() => {
        router.push("/lender/auth/forgot-password");
      }, 2000);
    }
  }, [userId, router]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!userId) {
      ...
      setError(errorMsg);
      toast.error(errorMsg);
      setTimeout(() => router.push("/lender/auth/forgot-password"), 2000);
      return;
    }
        ...
        <div className="w-full max-w-[765px]">
          ...
          <div className="mt-6 text-center">
            <p className="text-sm text-gray-500">
              ...
              <button
                onClick={() => router.push("/lender/auth/signin")}
                className="text-[#1e5ba8] hover:underline"
              >
                Sign in
              </button>
            </p>
